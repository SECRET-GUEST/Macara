...
from PyQt5.QtWidgets import  QWidget, QHBoxLayout,QVBoxLayout, QPushButton, QSpinBox, QFileDialog, QLabel, QComboBox, QMessageBox #,QApplication
...
class ImageResizer(QWidget):
    ...
    def update_tooltip(self, index):
        tooltip_text = self.selection_combo.itemData(index)
        self.selection_combo.setToolTip(tooltip_text)
...
#|__] |  \ |  | |    |  | | |___ |  | |___    |__| ___] |___ |  \    | | \|  |  |___ |  \ |    |  | |___ |___ 
    ...
    def GUI(self):
        ...
        self.selection_combo.addItems(["Single File", "Folder", "With Subfolders"])
        self.selection_combo.setItemData(0, "Resize a single file.")
        self.selection_combo.setItemData(1, "Resize all files in a folder.")
        self.selection_combo.setItemData(2, "Resize all files in the folders and subfolders.")

        # Connect the signal to update the tooltip when the index changes
        self.selection_combo.currentIndexChanged.connect(self.update_tooltip)
        
        # Display the tooltip for the first item
        ...
        # Create a QHBoxLayout for the width label and spinbox
        self.widthLay = QHBoxLayout()
        self.widthLay.addWidget(width_label)
        self.widthLay.addWidget(self.width_spinbox)

        # Add the QHBoxLayout for width to the QVBoxLayout
        self.resLay.addLayout(self.widthLay)

        # Create a QHBoxLayout for the height label and spinbox
        self.heightLay = QHBoxLayout()
        self.heightLay.addWidget(height_label)
        self.heightLay.addWidget(self.height_spinbox)

        # Add the QHBoxLayout for height to the QVBoxLayout
        self.resLay.addLayout(self.heightLay)
