...
class cypunk3MainWindow(QMainWindow):
    resized = pyqtSignal()
    ...
    def mwgui(self, title, window_size, btn_minimize, btn_show, stylesheet_path):
        ...
        self.setWindowFlags( Qt.FramelessWindowHint)  # set window flags Qt.WindowStaysOnTopHint to stay top
    ...
    # Update the central widget's geometry to match the window size
    def update_central_widget_geometry(self):
        self.central_widget.setGeometry(0, 0, self.width(), self.height())

    # Handle the mouse press event for resizing and moving the window
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            self.mouse_pressed = True  # Indicate that the mouse is being pressed
            self.mouse_position = event.globalPos() - self.pos()  # Store the mouse position

            # Check which edge of the window is being pressed for resizing
    ...
    # Handle the mouse release event to stop resizing/moving the window
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            self.mouse_pressed = False  # Indicate that the mouse has been released
            # Reset all resizing flags
            self.resizing_right = False
            self.resizing_bottom = False
            self.resizing_left = False
            self.resizing_top = False
            self.update_cursor(event)  # Update the cursor icon
            event.accept()



    # Handle the mouse move event to resize/move the window
    def mouseMoveEvent(self, event):
        self.update_cursor(event)  # Update the cursor icon

        if self.mouse_pressed:
            # Perform the appropriate resizing or moving action
    ...
    # Update the cursor icon based on the mouse position relative to the window's edges
    def update_cursor(self, event):
        if self.mouse_pressed:
            ...
            elif abs(self.height() - event.pos().y()) <= 10 or abs(event.pos().y()) <= 10: # This line seems redundant, consider removing or modifying it
                self.setCursor(Qt.SizeVerCursor)
            else:
